...
const App = () => {
  ...
  const searchMovies = async (title) => {
    try {
      ...
      setError(null);
      const response = await fetch(`${API_URL}&query=${encodeURIComponent(cleanTitle)}`);
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

      const data = await response.json();
      setMovies(data.results || []);
      if (data.results.length === 0) setError("No movies found for the given search term.");
    } catch (error) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      await axios.post(`${BACKEND_URL}/sign-out`, {}, {
        headers: {
          ...
          <div className="form-box">
            ...
            <form onSubmit={handleAuthSubmit}>
              ...
              <button type="submit" disabled={loading}>
                {loading ? "Please wait..." : formType === "login" ? "Login" : "Sign Up"}
              </button>
            </form>
            <p>
              {formType === "login"
              ...
              <span onClick={() => setFormType(formType === "login" ? "signup" : "login")}>
